(function () {
  ...
  function renderRung(rung, index, viewportWidth = 900, showIndex = false) {
    ...
    let top = 0;
    rows.forEach((row,i)=>{const cy=top+y;const startX=i?railL+14:railL;const endX=i<rows.length-1?railR-14:railR;parts.push(`<line class="wire" x1="${startX}" y1="${cy}" x2="${nodeL}" y2="${cy}"/>`);renderRoot(row,nodeL,cy,drawW,parts);parts.push(`<line class="wire" x1="${nodeR}" y1="${cy}" x2="${endX}" y2="${cy}"/>`);if(i<rows.length-1){const turnY=top+rowHeights[i]-18;parts.push(`<line class="wire" x1="${railR}" y1="${cy+8}" x2="${railR}" y2="${turnY}"/>`);continuationDown(parts,railR,cy+8,turnY);continuationArrow(parts,railR-14,railL+14,turnY);parts.push(`<line class="wire" x1="${railL}" y1="${turnY}" x2="${railL}" y2="${top+rowHeights[i]+y-8}"/>`);}top+=rowHeights[i];});
    parts.push('</svg>');
